...
class new_Random {
    ...
    choice(list, pickOnce = true, amount = 1) {
        // Check for errors
        if (!Array.isArray(list) || list.length == 0 || typeof list != 'object')
            throw new Error("Invalid value for argument 'list'.");
        if (typeof pickOnce != 'boolean')
            throw new Error("Invalid value for argument 'pickOnce'.");
        if (typeof amount != 'number' || isNaN(amount) || !Number.isSafeInteger(amount) || amount <= 0)
            throw new Error("Invalid value for argument 'amount'.");
        // Main function
     ...
     */
    randString(length, charListString = undefined) {
...
class new_listTools {
    ...
    arrayToChunks(list, chunkSize) {
        ...
        if (chunkSize == undefined || typeof chunkSize != 'number' || !Number.isInteger(chunkSize) || chunkSize <= 0)
            throw new Error("Chunk size must me a valid positive integer > 0");
    ...
    */
    all(from_list, list_in, caseSensitive = true) {
        ...
        for (let i in from_list) {
            if (list_in.indexOf(from_list[i]) == -1)
                return false;
        }
        return true;
    }
...
function hasSameKeys(pattern, obj) {
    ...
    for (let i = 0; i < objKeys.length; i++) {
        if (!patternKeys.includes(objKeys[i]))
            return false;
    }
    return true;
}
...
function hasSameKeysList(patternList, obj) {
    let objKeys = Object.keys(obj);
    if (objKeys.length !== patternList.length)
        return false;
    for (let i = 0; i < objKeys.length; i++) {
        if (!patternList.includes(objKeys[i]))
            return false;
    }
    return true;
}
...
function parseMillisecondsFromTimeString(str) {
    ...
    if (weeks) {
        ...
        ;
        total += parseInt(weeks[1]) * 604800 * 1000;
    }
    if (days) {
        if (isNaN(total)) {
            total = 0;
        ...
        ;
        total += parseInt(days[1]) * 86400 * 1000;
    }
    if (hours) {
        if (isNaN(total)) {
            total = 0;
        ...
        ;
        total += parseInt(hours[1]) * 3600 * 1000;
    }
    if (minutes) {
        if (isNaN(total)) {
            total = 0;
    ...
    for (let link in _links) {
        for (let scamLink in blacklistedLinks) {
            ...
            */
            ...
            if (!s_to_test.startsWith("https://")) {
                s_to_test = `https://${s_to_test}`;
            }
 ...
 * @returns {String}
 */
export const capitalize = (str) => { return `${str[0].toUpperCase()}${str.slice(1)}`; };
/**
    ...
    constructor() {
        ...
        this.emit = (callName, ...datasList) => {
            ...
            for (let i in this.eventsNames[callName]) {
                try {
                    this.eventsNames[callName][i](...datasList);
                }
                catch (e) {
                    console.log(e);
                }
